...
export const QRScannerModal: React.FC<QRScannerModalProps> = ({ visible, onClose, onScanned }) => {
  ...
  const translateY = lineAnim.interpolate({ inputRange: [0, 1], outputRange: [0, frameSize - 4] });
        ...
        <View style={styles.cameraWrap}>
          {permission && permission.granted === false ? (
            ...
            <CameraView
              ...
              onBarcodeScanned={scanned ? undefined : ((result: { data: string }) => handleBarCodeScanned({ data: result.data }))}
            />
          )}

          <View style={[styles.overlay, { top: 0, left: 0, right: 0, height: frameTop }]} />
          <View style={[styles.overlay, { top: frameTop, left: 0, width: frameLeft, height: frameSize }]} />
          <View style={[styles.overlay, { top: frameTop, right: 0, width: frameLeft, height: frameSize }]} />
          <View style={[styles.overlay, { top: frameTop + frameSize, left: 0, right: 0, bottom: 0 }]} />

          <View style={[styles.frame, { width: frameSize, height: frameSize, top: frameTop, left: frameLeft }]}>
          ...
          <View style={styles.headerTextWrap}>
            ...
            <Text style={styles.instructionText}>Align the QR within the frame to start the job</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  permissionBtn: { marginTop: 16, backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10 },
  permissionBtnText: { color: '#FFFFFF' },
  overlay: { position: 'absolute', backgroundColor: 'rgba(0,0,0,0.45)' },
  frame: { position: 'absolute', borderRadius: 18, borderWidth: 2, borderColor: '#22D3EE', overflow: 'hidden' },
  scanLine: { position: 'absolute', left: 0, right: 0, height: 3, backgroundColor: '#22D3EE', opacity: 0.9 },
  corner: { position: 'absolute', width: 24, height: 24, borderColor: '#22D3EE' },
  ...
  headerTextWrap: { position: 'absolute', top: 56, left: 0, right: 0, alignItems: 'center' },
  headerText: { color: '#FFFFFF', fontSize: 18, fontWeight: '700' },
  instructionText: { color: '#E5E7EB', fontSize: 13, marginTop: 6 },
  bottomWrap: { position: 'absolute', left: 0, right: 0, bottom: 40, alignItems: 'center' },
  cancelBtn: { backgroundColor: '#FFFFFF', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 14 },
